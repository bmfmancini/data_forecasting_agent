...
def test_edit_remove_restore_and_reset():
    ...
    result["section_edits"] = apply_section_edit(
        result, "1", "save", section["version"], "Outlook", "My **revised** wording."
    )
    ...
    section = report_sections(result)[1]
    result["section_edits"] = apply_section_edit(
        result, "1", "remove", section["version"]
    )
    assert "Outlook" not in effective_markdown(result)
    assert "[VISUAL:FORECAST]" not in effective_markdown(result)
    result["section_edits"] = apply_section_edit(
        result, "1", "restore", report_sections(result)[1]["version"]
    )
    assert "My **revised** wording." in effective_markdown(result)
    result["section_edits"] = apply_section_edit(
        result, "1", "reset", report_sections(result)[1]["version"]
    )
...
def test_legacy_unsectioned_reports_can_be_edited():
    ...
    assert apply_section_edit(
        result, "0", "save", section["version"], "Report", "Updated"
    )
