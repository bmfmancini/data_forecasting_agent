...
def test_owned_section_edits_persist_without_changing_original(tmp_path, monkeypatch):
    ...
    assert not report_service.edit_report_section_for_user(
        report_id, 999, "0", "remove", version
    )
    assert report_service.edit_report_section_for_user(
        report_id, 1, "0", "save", version, "Summary", "My wording"
    )
    with pytest.raises(EditConflict):
        report_service.edit_report_section_for_user(
            report_id, 1, "0", "save", version, "Summary", "Stale wording"
        )
    row = dict(
        connection.execute(
            "SELECT * FROM forecast_reports WHERE id = ?", (report_id,)
        ).fetchone()
    )
