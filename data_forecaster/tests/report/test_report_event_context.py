...
def test_valentines_is_report_observance():
    context, _, notes = build_event_context(
        {"holidays_country": "US"}, None, series("2025-02-13", [5, 20, 6])
    )
...
def test_monthly_flat_and_gapped_series_do_not_claim_daily_peaks():
    ...
    for forecast in [
        series("2025-01-14", [5, 20, 6], "31D"),
        series("2025-02-13", [5, 5, 5]),
        series("2025-02-12", [5, 20, 6], "2D"),
    ]:
...
def test_custom_events_and_covariate_values_are_retained():
    context, _, notes = build_event_context(
        {
            "known_events": [
                {"date": "2025-02-14", "type": "promotion", "label": "Half-price sale"}
            ],
            "known_covariates": {"price": {"2025-02-14": 5}},
        },
        None,
        series("2025-02-13", [5, 20, 6]),
    )
...
def test_regional_calendar_changes_report_matches():
    ...
    _, _, qc = build_event_context(
        {"holidays_country": "CA", "holidays_subdivision": "QC"}, None, forecast
    )
    _, _, on = build_event_context(
        {"holidays_country": "CA", "holidays_subdivision": "ON"}, None, forecast
    )
    assert qc
    assert not on
