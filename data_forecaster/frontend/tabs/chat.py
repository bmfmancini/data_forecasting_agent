...
def render_chat_tab(info):
    ...
    st.info(
        "Ask questions about time series data analysis or general forecasting concepts."
    )
        ...
        with st.chat_message(message["role"]):
            ...
            if message.get("viz"):
                ...
                if viz["type"] == "pie":
                    ...
                    visualizer.render_from_config(
                        viz["data"],
                        key=f"chat_viz_{len(st.session_state.chat_history)}",
                    )
                ...
                with st.spinner("Thinking..."):
                    ...
                    if resp.status_code == 200:
                        ...
                        # Handle dynamic visualizations from LLM
                            ...
                            viz_data = data["visualization_data"]
                            if visualizer.render_from_config(
                                viz_data,
                                key=f"chat_viz_new_{len(st.session_state.chat_history)}",
                            ):
                        ...
                        # Try to parse visualization from the answer text as a fallback
                            ...
                            if parsed_viz:
                                if visualizer.render_from_config(
                                    parsed_viz,
                                    key=f"chat_viz_parsed_{len(st.session_state.chat_history)}",
                                ):
                                    viz_payload = {
                                        "type": "dynamic",
                                        "data": parsed_viz,
                        ...
                        st.session_state.chat_history.append(
                            {"role": "assistant", "content": answer, "viz": viz_payload}
                        )
                    else:
                        st.error(
                            f"Error: {resp.json().get('detail', 'Failed to get response.')}"
                        )
