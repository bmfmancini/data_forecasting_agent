...
def build_event_context(
    ...
    if str(options.get("holidays_country", "")).upper() in {
        "US",
        "CA",
        "GB",
        "IE",
        ...
        events.extend(
            {"date": f"{year}-02-14", "type": "observance", "label": "Valentine's Day"}
            for year in years
        )
    events = [
        e
        for e in events
        if start.normalize() <= pd.Timestamp(e["date"]) <= end.normalize()
    ]
    events = sorted(
        {(e["date"], e["label"], e["type"]): e for e in events}.values(),
    ...
    def match(series: pd.Series, label: str) -> tuple[list[dict[str, Any]], list[str]]:
        ...
        for i in range(1, len(series) - 1):
            ...
            if idx[i] - idx[i - 1] != pd.Timedelta(days=1) or idx[i + 1] - idx[
                i
            ] != pd.Timedelta(days=1):
                continue
            ...
            kind = (
                ...
                else "local trough" if value < min(left, right) else None
            )
            ...
            matches.append(
                {
                    ...
                    "date": day,
                    "value": float(value),
                    "previous_value": float(left),
                    "next_value": float(right),
                    "pattern": kind,
                    "events": by_date[day],
                }
            )
        ...
        for item in matches[:12]:
            ...
            notes.append(
                f"{label} {item['pattern']} of {item['value']:g} on {item['date']} coincides with {names}. This timing may be relevant to interpretation; it does not establish that the event caused the movement. Forecast values have not been adjusted for this note."
            )
    ...
    ordered = list(
        {
            (e["date"], e["label"], e["type"]): e for e in matched_events + events
        }.values()
    )
    ...
    for name, values in list(covariates.items())[:10]:
        if isinstance(values, dict):
            ...
            covariate_context[name] = {
                "values": dict(points[:24]),
                "omitted_values": max(0, len(points) - 24),
            }
    context = {
